...
@section('content')
    ...
    @if (session()->has('delete_outgoings'))
        <script>
    ...
    @if (session()->has('restore_outgoings'))
        <script>
            window.onload = function() {
                notif({
        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                        ...
                        <a href="outgoings/create" class="modal-effect btn btn-sm btn-primary" style="color:white"><i
    ...
    <!-- حذف الفاتورة -->
    <div class="modal fade" id="delete_outgoings" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form action="{{ route('outgoings.destroy', 'test') }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                </div>
                <div class="modal-body">
                    هل انت متاكد من عملية الحذف ؟
                    <input type="hidden" name="outgoings_id" id="outgoings_id" value="">
...
@section('js')
    ...
    <script>
        $('#delete_outgoings').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget)
            var outgoings_id = button.data('outgoings_id')
            var modal = $(this)
            modal.find('.modal-body #outgoings_id').val(outgoings_id);
        })

    </script>

    <script>
        $('#Transfer_outgoings').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget)
            var outgoings_id = button.data('outgoings_id')
            var modal = $(this)
            modal.find('.modal-body #outgoings_id').val(outgoings_id);
